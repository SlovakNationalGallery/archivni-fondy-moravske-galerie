<template>
    <layout>
        <div class="bg-gray-200 px-6 lg:px-16 py-8" v-if="!hideFilter">
            <div class="-mx-2">
                <div class="flex flex-wrap -my-2">
                    <div
                        class="my-2 px-2 w-full lg:w-1/3"
                        v-for="(filterOptions, key) in options.filter"
                        :key="key"
                    >
                        <facet
                            :label="labels[key]"
        ...
        <template v-slot:container>
            <div class="my-6 lg:my-8 text-lg" v-if="total">
                {{ total }} {{ totalLabel }}
            </div>
            <div class="my-6 lg:my-8 text-center text-lg" v-else-if="!loading">
                Nebyly nalezeny žádné záznamy
            </div>
            <div class="my-6 lg:my-8" v-else>
                <img class="mx-auto" src="/images/loader.svg" />
            </div>

            <div
                class="-mx-2 -my-4"
                item-selector="[data-masonry-tile]"
            ...
            >
                <div class="flex flex-wrap mb-10">
                    <div
                        v-masonry-tile
                        class="px-2 py-4 w-1/2 lg:w-1/4 xl:w-1/5"
                    ...
                    >
                        ...
                        >
                            <img
                                @load="debouncedRedraw"
                                class="w-full"
                                :src="imagePreviewUrl(item.images[0], 400)"
                            />
...
export default {
    ...
    methods: {
        ...
        fetchItems() {
            ...
            params.set("size", 20);
            return axios.get(`/api/items`, { params }).then(({ data }) => {
        ...
        filterParams() {
            const queryString = this.$router.options.stringifyQuery(
                this.$route.query
            );
